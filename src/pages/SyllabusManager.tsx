...
import { useStudyData, ChapterDifficulty, ChapterLength } from '@/context/StudyDataContext';
...
import { ArrowLeft, Plus, BookOpen, Edit, Trash2, ChevronDown, ChevronRight } from 'lucide-react';
...
const SyllabusManager = () => {
  ...
  const [newChapterDialog, setNewChapterDialog] = useState<string | null>(null);
  const [newTopicDialog, setNewTopicDialog] = useState<{ subjectId: string; chapterId: string } | null>(null);
  ...
  const handleAddChapter = (subjectId: string) => {
    ...
    addChapter(subjectId, {
      ...
      length: chapterForm.length,
      topics: []
    });

    toast.success('Chapter added successfully!');
    setNewChapterDialog(null);
    setChapterForm({ name: '', difficulty: 'medium', length: 'medium' });
  };
  ...
  const handleAddTopic = () => {
    ...
    addTopic(newTopicDialog.subjectId, newTopicDialog.chapterId, {
      name: topicForm.name,
      status: 'pending',
    ...
    setTopicForm({ name: '', timeAllocated: 60, notes: '' });
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Organize your subjects, chapters, and topics with completion tracking</p>
              </div>
            </div>
                  ...
                  {expandedSubjects.has(subject.id) && (
                    <div className="border-t border-border p-4 bg-muted/20">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <Dialog open={newChapterDialog === subject.id} onOpenChange={(open) => setNewChapterDialog(open ? subject.id : null)}>
                          <DialogTrigger asChild>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              <DialogTitle>Add New Chapter to {subject.name}</DialogTitle>
                            </DialogHeader>
                            <div className="space-y-4">
                              <div>
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) => setChapterForm({ ...chapterForm, difficulty: e.target.value as ChapterDifficulty })}
                                >
                              ...
                              <div>
                                ...
                                <select
                                  className="w-full mt-1 px-3 py-2 bg-background border border-input rounded-md"
                                  value={chapterForm.length}
                                  onChange={(e) => setChapterForm({ ...chapterForm, length: e.target.value as ChapterLength })}
                                >
                              ...
                              <Button onClick={() => handleAddChapter(subject.id)} className="w-full gradient-primary text-white">
                                Add Chapter
                              </Button>
                            </div>
                          </DialogContent>
                      ...
                      {subject.chapters.length === 0 ? (
                        <p className="text-muted-foreground text-sm text-center py-4">No chapters added yet</p>
                      ) : (
                        <div className="space-y-3">
                          {subject.chapters.map((chapter) => (
                            <div key={chapter.id} className="bg-card rounded-lg border border-border">
                              <div
                              ...
                              >
                                    ...
                                    <div>
                                      ...
                                      <div className="flex gap-2 mt-1">
                                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                                          chapter.difficulty === 'hard' ? 'bg-red-500/20 text-red-500' :
                                          chapter.difficulty === 'medium' ? 'bg-yellow-500/20 text-yellow-500' :
                                ...
                                <Progress value={chapter.progress} className="mt-2 h-1" />
                              </div>
                              ...
                              {expandedChapters.has(chapter.id) && (
                                <div className="border-t border-border p-3 bg-muted/10">
                                  <div className="flex justify-between items-center mb-3">
                                    ...
                                    <Dialog
                                      open={newTopicDialog?.subjectId === subject.id && newTopicDialog?.chapterId === chapter.id}
                                      onOpenChange={(open) => setNewTopicDialog(open ? { subjectId: subject.id, chapterId: chapter.id } : null)}
                                    >
                                      <DialogTrigger asChild>
                                      ...
                                      <DialogContent>
                                        ...
                                        <div className="space-y-4">
                                          ...
                                          <div>
                                            ...
                                            <Textarea
                                              placeholder="Any notes or important points..."
                                  ...
                                  {chapter.topics.length === 0 ? (
                                    <p className="text-muted-foreground text-xs text-center py-2">No topics added yet</p>
                                  ) : (
                                        ...
                                        >
                                          ...
                                          <div className="flex items-center gap-2">
                                            <select
                                              value={topic.status}
                                              onChange={(e) => {
                                                updateTopicStatus(subject.id, chapter.id, topic.id, e.target.value as any);
